*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVSA01PRO
*   generation date: 2022.07.21 at 18:03:36
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVSA01PRO           .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVSA01PRO'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVSA01PRO_EXTRACT
TOTAL = ZVSA01PRO_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVSA01PRO'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVSA01PRO-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVSA01PRO_EXTRACT
TOTAL = ZVSA01PRO_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVSA01PRO-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVSA01PRO'
...
TOTAL = ZVSA01PRO_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVSA01PRO-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVSA01PRO-UPD_FLAG NE SPACE.
